...
################################################################################
# -*- coding: utf-8 -*-
# generated by wxGlade 0.6.3 on Fri Nov 12 20:42:28 2010
...
class CopyFrame(wx.Frame):
    ...
    def __do_layout(self):
        ...
        sizer_7.Add(sizer_1, 1, wx.EXPAND, 0)
    ...
    def evt_splitter1_resize(self, event): # wxGlade: CopyFrame.<event_handler>
        idx = self.filelst.GetFirstSelected()
        if idx != -1:
            self.previewpane.display(self.filelst.GetItemText(idx))
        event.Skip()

    def evt_filelst_select(self, event): # wxGlade: CopyFrame.<event_handler>
        idx = self.filelst.GetFirstSelected()
        if idx != -1:
            self.previewpane.display(self.filelst.GetItemText(idx))
        event.Skip()

    def evt_cancelbtn(self, event): # wxGlade: CopyFrame.<event_handler>
        self.Close()
        event.Skip()

    def evt_okbtn(self, event): # wxGlade: CopyFrame.<event_handler>
        self.handler(self.date, self.namevalue.GetValue())
        self.Close()
        event.Skip()

    def evt_neverbtn(self, event): # wxGlade: CopyFrame.<event_handler>
        import os
        path = self.destvalue.GetValue() + "/" + self.namevalue.GetValue().replace("<name>", "never_import")
        os.mkdir(path)
        event.Skip()

# end of class CopyFrame
